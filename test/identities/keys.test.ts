  ...
  describe("Key Management", () => {
    describe("Read key methods", () => {
      ...
      it("should retrieve existing keys with given purpose", async () => {
        ...
        const keys = await aliceIdentity.getKeysByPurpose(
      ...
      it("should return true if a key has a given purpose", async () => {
        ...
        const hasPurpose = await aliceIdentity.keyHasPurpose(
          aliceKeyHash,
          KeyPurposes.MANAGEMENT,
        );
    ...
    describe("Add key methods", () => {
      describe("when calling as a non-MANAGEMENT key", () => {
        it("should revert because the signer is not a MANAGEMENT key", async () => {
          ...
          await expect(
            aliceIdentity
              .connect(bobWallet)
              .addKey(bobKeyHash, KeyPurposes.MANAGEMENT, KeyTypes.ECDSA),
      ...
      describe("when calling as a MANAGEMENT key", () => {
        it("should add the purpose to the existing key", async () => {
          ...
          await aliceIdentity
            .connect(aliceWallet)
            .addKey(aliceKeyHash, KeyPurposes.ACTION, KeyTypes.ECDSA);
          const aliceKey = await aliceIdentity.getKey(aliceKeyHash);
          expect(aliceKey.key).to.equal(aliceKeyHash);
        ...
        it("should add a new key with a purpose", async () => {
          const { aliceIdentity, bobWallet, aliceWallet } = await loadFixture(
            deployIdentityFixture,
          );
        ...
        it("should revert because key already has the purpose", async () => {
          ...
          await expect(
            aliceIdentity
              .connect(aliceWallet)
              .addKey(aliceKeyHash, KeyPurposes.MANAGEMENT, KeyTypes.ECDSA),
          ).to.be.revertedWithCustomError(
            aliceIdentity,
            "KeyAlreadyHasPurpose",
          );
        });
      });
    });
      ...
      describe("when calling as a non-MANAGEMENT key", () => {
        it("should revert because the signer is not a MANAGEMENT key", async () => {
          ...
          await expect(
            aliceIdentity
              .connect(bobWallet)
              .removeKey(aliceKeyHash, KeyPurposes.MANAGEMENT),
          ).to.be.revertedWithCustomError(
            aliceIdentity,
            "SenderDoesNotHaveManagementKey",
          );
      ...
      describe("when calling as a MANAGEMENT key", () => {
        ...
        it("should revert because key does not exists", async () => {
          ...
          await expect(
            aliceIdentity
              .connect(aliceWallet)
              .removeKey(bobKeyHash, KeyPurposes.ACTION),
        ...
        it("should revert because key does not have the purpose", async () => {
          ...
          await expect(
            aliceIdentity
              .connect(aliceWallet)
              .removeKey(aliceKeyHash, KeyPurposes.ACTION),
          ).to.be.revertedWithCustomError(
            aliceIdentity,
        ...
        it("should handle edge case where key is not found in _keysByPurpose array", async () => {
          const { aliceIdentity, aliceWallet, bobWallet } = await loadFixture(
            deployIdentityFixture,
          );

          // Add bob as a management key first
          const bobKeyHash = ethers.keccak256(
            ethers.AbiCoder.defaultAbiCoder().encode(
              ["address"],
              [bobWallet.address],
          ...
          // Add bob as an action key as well
          await aliceIdentity
            .connect(aliceWallet)
            .addKey(bobKeyHash, KeyPurposes.ACTION, KeyTypes.ECDSA);

          // Now remove the management purpose - this should trigger the edge case
          // where the key exists but might not be found in the array search
          await aliceIdentity
            .connect(aliceWallet)
            .removeKey(bobKeyHash, KeyPurposes.MANAGEMENT);

          // Verify the key still has the ACTION purpose
          const bobKey = await aliceIdentity.getKey(bobKeyHash);
          expect(bobKey.purposes).to.deep.equal([KeyPurposes.ACTION]);
          expect(bobKey.keyType).to.equal(KeyTypes.ECDSA);
        });
      });
    });
