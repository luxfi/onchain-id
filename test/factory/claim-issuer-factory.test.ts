...
describe("ClaimIssuerFactory", () => {
  ...
  it("should deploy a new ClaimIssuer contract to a predetermined address", async () => {
    ...
    await expect(tx)
      .to.emit(claimIssuerFactory, "ClaimIssuerDeployed")
      .withArgs(deployer.address, claimIssuer);
    ...
    const expectedAddress =
      "0x" + ethers.keccak256(ethers.encodeRlp([proxy, "0x01"])).slice(-40);
  ...
  it("should emit an event when an address is unblacklisted", async () => {
    await claimIssuerFactory
      .connect(deployer)
      .blacklistAddress(alice.address, true);

    const tx = await claimIssuerFactory
      .connect(deployer)
      .blacklistAddress(alice.address, false);
    await expect(tx)
      .to.emit(claimIssuerFactory, "Blacklisted")
  ...
  it("should emit an event when the implementation is updated", async () => {
    ...
    await expect(tx)
      ...
      .withArgs(await claimIssuerImplementation.getAddress(), alice.address);
  });
});
